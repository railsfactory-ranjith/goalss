...
Object.extend(Control.Modal.prototype,{
	...
	initialize: function(element,options){
		...
		this.options = {
			...
			contents: false,
			loading: true, //display loading indicator
			fade: false,
			fadeDuration: 0.75,
			image: false,
		...
		this.position = function(event){
			if(this.options.position == 'absolute')
				...
				Control.Modal.container.setStyle({
					position: 'absolute',
					top: xy[1] + (typeof(this.options.offsetTop) == 'function' ? this.options.offsetTop() : this.options.offsetTop) + '%',
					left: xy[0] + (typeof(this.options.offsetLeft) == 'function' ? this.options.offsetLeft() : this.options.offsetLeft) + '%'
				});
			}
			if(Control.Modal.ie){
	...
	showLoadingIndicator: function(){
		if(this.options.loading){
			Control.Modal.loadingTimeout = window.setTimeout(function(){
				...
				Control.Modal.loading.update('<img id="modal_loading" src="/images/circle_animation.gif"/>');
